...
The borrower agrees to collateralize the loan with Lenovo G480 laptop, Hisense 32 inch screen TV, iPad mini 2 Wi-Fi+Cellular.  

The borrower agrees to pay for registration fees to register this document with the Hong Kong Company Registry, and the fees will be added to the amounts owed to the lender.  The lender agrees to be responsible for the registration of the charges.  */}).toString().match(/[^]*\/\*([^]*)\*\/\}$/)[1];
    parties.set(obj);
};
...
function Schedule_A(obj) {
    // S.1
    obj.annual_interest_rate = 15.0;
    obj.compound_per_year = 12;
    obj.day_count_convention = "30/360US";

    obj.initial_date = new_date(2016, 3, 14);
    obj.initial_date_string = obj.initial_date.toDateString();
    obj.interval = [1, "month"];
    obj.currency = 'HKD';
    obj.initial_amount = 40000.00;
    obj.number_payments = 6;
    obj.charge_registration_fee = 390.00;
    obj.charge_release_fee = 190.00;
}
...
function Schedule_C(obj) {
	...
	{ 
	    name: "charge_registration_fee",
	    display: "Charge registration fee",
	    type: "number",
	    scenario: true
	},
	{ 
	    name: "charge_release_fee",
	    display: "Charge release fee",
	    type: "number",
	    scenario: true
	}
    ];
...
Schedule_C.prototype.payments = function(calc) {
    ...
    calc.add_to_balance({"on" : this.initial_date,
			 "amount" : this.charge_registration_fee,
			 "note" : "Charge registration fee"}); 
	...
	following_1st_of_month_exclusive(calc.add_duration(this.initial_date,
							   [this.interval[0] * (this.number_payments-1),
					  this.interval[1]]));

    calc.amortize({"on": this.initial_date,
		   "first_payment_date" : first_payment_date,
                   "amount": calc.remaining_balance(),
                   "payments" : this.number_payments,
                   "interval" : this.interval,
		   "payment_func" : payment_function});
    calc.add_to_balance({"on" : final_payment_date,
			 "amount" : this.charge_release_fee,
			 "note" : "Charge release fee"}); 
